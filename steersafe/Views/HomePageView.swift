    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                ...
                // Display red if z-axis acceleration is too high (using phone), otherwise green or grey based on state
            ...
            Text(viewModel.isWarningVisible ? "get off your phone!" : (viewModel.isDriving ? "stay focused" : "tap the wheel to start"))
                ...
                .padding(.top, 10)
                .transition(.scale)
                .foregroundColor(viewModel.isWarningVisible ? .red : (viewModel.isDriving ? Color(UIColor(red: 0.23, green: 0.86, blue: 0.57, alpha: 1.00)) : .gray))  // Red if using phone, green if driving, gray otherwise
